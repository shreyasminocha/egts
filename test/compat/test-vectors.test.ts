...
import {encryptBallot, EncryptionState} from '../../src/electionguard';
...
describe('compat', () => {
  ...
  test('can decode the sample data', async () => {
    ...
    content = await fs.readFile(
      path.join(dir, 'encryption_devices', 'device_187970240912820.json'),
      'utf-8'
    );
    const deviceJson = JSON.parse(content);
    const possiblyDevice =
      getCoreCodecsForContext(groupContext).encryptionDeviceCodec.decode(
        deviceJson
      );
    const device = eitherRightOrFail(possiblyDevice);
    ...
    const state = new EncryptionState(groupContext, manifest, context, false);
    const ourCiphertext = encryptBallot(
      state,
      plaintextBallot,
      device.cryptoHashElement,
